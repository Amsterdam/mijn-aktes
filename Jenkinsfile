...
if (BRANCH == "master") {
    ...
    node {
        stage('Push acceptance image') {
            tryStep "image tagging", {
                docker.withRegistry("${DOCKER_REGISTRY_HOST}",'docker_registry_auth') {
                    docker.image("mijnams/${PROJ_NAME}:${env.BUILD_NUMBER}").pull()
                    ...
                    retagAndPush("mijnams/${PROJ_NAME}", "${env.BUILD_NUMBER}", "acceptance")
                }
            }
        }
    }
            ...
            tryStep "deployment", {
                build job: 'Subtask_Openstack_Playbook',
                    parameters: [
                        ...
                        [$class: 'StringParameterValue', name: 'PLAYBOOKPARAMS', value: "-e cmdb_id=app_mijn-aktes"]
                    ]
            }
        }
    }
    ...
    stage('Waiting for approval') {
        slackSend channel: '#ci-channel', color: 'warning', message: "${PROJ_NAME} is waiting for Production Release - please confirm"
    ...
    node {
        stage('Push production image') {
            tryStep "image tagging", {
                docker.withRegistry("${DOCKER_REGISTRY_HOST}",'docker_registry_auth') {
                    docker.image("mijnams/${PROJ_NAME}:${env.BUILD_NUMBER}").pull()
                    retagAndPush("mijnams/${PROJ_NAME}", "${env.BUILD_NUMBER}", "production")
                    retagAndPush("mijnams/${PROJ_NAME}", "${env.BUILD_NUMBER}", "latest")
                }
            }
        }
    }
